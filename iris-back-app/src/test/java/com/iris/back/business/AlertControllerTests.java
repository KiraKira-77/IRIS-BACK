...
class AlertControllerTests {
  ...
  void listReturnsAlertPayload() throws Exception {
    when(alertService.list(any(AlertQuery.class))).thenReturn(PageResponse.of(
        1,
        1,
        10,
        List.of(new AlertEventDto(
            "work-order-sync-8001",
            "OMS工单",
            "critical",
            "OMS工单同步失败",
            "OMS接口超时",
    ...
    mockMvc.perform(get("/api/v1/alerts")
        ...
        .andExpect(jsonPath("$.data.records[0].source").value("OMS工单"))
        .andExpect(jsonPath("$.data.records[0].acknowledged").value(false));
  }

  @Test
  @WithMockUser(username = "admin", roles = "PLATFORM_ADMIN")
  void acknowledgeRouteDelegatesToAlertService() throws Exception {
    mockMvc.perform(put("/api/v1/alerts/work-order-sync-8001/ack"))
        .andExpect(status().isOk());

    verify(alertService).acknowledge("work-order-sync-8001");
  }
}
